...
# Configure matplotlib for PGF backend if needed
if AnalysisSettings.PLOTTING_FORMAT == "pgf":
    matplotlib.rcParams.update({
        "pgf.texsystem": "pdflatex",
        "text.usetex": False,  # Use PGF, not full LaTeX
        "pgf.rcfonts": False,  # Use matplotlib's font settings
    })
...
def extract_state_dict(obj) -> Dict[str, torch.Tensor]:
    ...
    raise ValueError(
        "Could not locate a tensor state dict (tried delta_state_dict/state_dict/model_state_dict)."
    )
...
def plot_drift_comparison_subplots(
    ...
    # Avoid newlines in titles for PGF compatibility - use single line format
    title1 = f"A (drifted) $-$ B (drifted) ({metric})"
    ...
    title2 = f"A: drifted $-$ non-drifted ({metric})"
    ...
    title3 = f"B: drifted $-$ non-drifted ({metric})"
    ...
    # Adjust subplot spacing: reduce left/right padding, add bottom padding for x-axis labels
    # right=0.92 leaves space for colorbars (positioned with pad=0.02 relative to axes)
    fig.subplots_adjust(left=0.02, right=0.96, bottom=0.21, top=0.92, wspace=0.25)
...
def main(dir_a: str = None, dir_b: str = None) -> None:
    ...
    if not dir_a:
        dir_a = "./fl_runs/MNIST/label_swapping/incremental/5-6_bidirectional_cf-0.5/update_datasets_run_2026-01-26_09-27-18_fedavg_cb1f65f9-7fdf-4b9d-a762-718ab4f021d1/updates/client_updates"
    if not dir_b:
        dir_b = "./fl_runs/MNIST/rotation/incremental/all_classes_rot_65_cf-0.5/update_datasets_run_2026-01-26_09-29-33_fedavg_cb1f65f9-7fdf-4b9d-a762-718ab4f021d1/updates/client_updates"
    ...
    if not keys:
        raise SystemExit(
            "No matching (client, round) pairs found between the two folders (or after filtering by --round)."
        )
    ...
    if has_drift_info:
        ...
        print(
            f"Found {len(drifted_client_ids)} drifted clients and {len(non_drifted_client_ids)} non-drifted clients"
        )

        # Create drift comparison plots per round
        for rnd in sorted(set(k.round for k in keys)):
            drifted_keys_rnd = [
                k for k in keys if k.round == rnd and k.client in drifted_client_ids
            ]
            non_drifted_keys_rnd = [
                k for k in keys if k.round == rnd and k.client in non_drifted_client_ids
            ]
            ...
            if (
                mat_a_drifted_list
                and mat_a_non_drifted_list
                and mat_b_drifted_list
                and mat_b_non_drifted_list
            ):
    ...
    for rnd, payload in sorted(by_round.items()):
        ...
        if signed_metric:
            vmax_round = (
                float(np.max(np.abs(np.concatenate([mat_a.ravel(), mat_b.ravel(), mat_d.ravel()]))))
                or 1.0
            )
        else:
    ...
    if has_drift_info:
        print(
            "  - drift_comparison_round_<r>.png (3-panel: A-B drifted | A drifted-nondrifted | B drifted-nondrifted)"
        )
